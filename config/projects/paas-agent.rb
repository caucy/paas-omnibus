...
name 'paas-agent'
maintainer 'PaasInsight Packages <package@paasinsight.com>'
homepage 'http://www.paasinsight.com'
...
description 'PassInsight Monitoring Agent
 The PaasInsight Monitoring Agent is a lightweight process that monitors system
 processes and services, and sends information back to your PaasInsight account.
 .
 This package installs and runs the advanced Agent daemon, which queues and
 forwards metrics from your applications as well as system services.
 .
 See http://www.paasinsight.com/ for more information
'
...
# .deb specific flags
package :deb do
  vendor 'PaasInsight <info@paasinsight.com>'
  epoch 1
...
# .rpm specific flags
package :rpm do
  vendor 'PaasInsight <package@paasinsight.com>'
  epoch 1
  dist_tag ''
  license 'Simplified BSD License'
...
package :pkg do
  identifier 'com.paasinsight.agent'
  signing_identity 'Developer ID Installer: PaasInsight, Inc. (JKFCB4CN7C)'
end
compress :dmg do
...
if linux?
  # Debian
  if debian?
    extra_package_file '/etc/init.d/paas-agent'
    extra_package_file '/lib/systemd/system/paas-agent.service'
  end

  # SysVInit service file
  if redhat?
    extra_package_file '/etc/rc.d/init.d/paas-agent'
  end

  if suse?
    extra_package_file '/etc/init.d/paas-agent'
    extra_package_file '/usr/lib/systemd/system/paas-agent.service'
  end

  # Supervisord config file for the agent
  extra_package_file '/etc/paas-agent/supervisor.conf'

  # Example configuration files for the agent and the checks
  extra_package_file '/etc/paas-agent/paasinsight.conf.example'
  extra_package_file '/etc/paas-agent/conf.d'

  # Custom checks directory
  extra_package_file '/etc/paas-agent/checks.d'
  ...
  extra_package_file '/usr/bin/paas-agent'
